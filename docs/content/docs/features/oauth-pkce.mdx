...
<Callout type="info">
  By default the issued key is **owned by the consenting user**. The
  consent page also lets the user pick any organization, team, or project
  they have permission to create keys for, so the same flow works for
  shared workspace keys. Keys can be revoked at any time from the API Keys
  page in Hadrian.
</Callout>
...
```json
{
  ...
  "scopes_supported": ["chat", "completions", "embeddings", "images", "audio", "files", "models", "admin"],
  "service_documentation": "https://hadrian.example.com/docs/features/oauth-pkce"
}
```

The issuer and endpoint URLs honour `X-Forwarded-Proto` / `X-Forwarded-Host`
when Hadrian is deployed behind a reverse proxy.
