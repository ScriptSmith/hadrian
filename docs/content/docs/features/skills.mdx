...
<Callout type="info">
  Skills are not injected into the system prompt. The directory lives in the
  `Skill` tool's description, so unused skills don't pollute every request.
</Callout>
...
<Callout type="warn">
  The unauthenticated GitHub API allows 60 requests/hour. Hadrian caches
  directory listings in `sessionStorage` for 10 minutes to soften this, and
  surfaces the remaining quota in the import modal.
</Callout>
...
- **Scripts are read-only.** The agent can *read* `scripts/` files through
