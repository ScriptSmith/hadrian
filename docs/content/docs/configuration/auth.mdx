...
governs policy evaluation for requests that are *already authenticated*, so it is never the
...
<Callout type="warn">
  Setting `allow_anonymous = true` while an auth mode is enabled opens **every** `/v1/*`
  endpoint to unauthenticated callers, including model inference, which can incur provider
  cost. Prefer leaving it `false` and issuing scoped API keys.
</Callout>

<Callout type="info">
  **Breaking change.** Earlier builds served credential-less `/v1/*` requests even when an
  auth mode was enabled (relying on `[auth.rbac.gateway].default_effect` as the only
  backstop). Deployments that depended on that behavior must now set `allow_anonymous = true`
  explicitly.
</Callout>
