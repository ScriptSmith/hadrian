...
description: Production-grade deployment configurations with Vault, observability, Keycloak, and high availability
---
...
<Callout type="warn">
  Save the unseal keys and root token securely! You'll need them to unseal Vault after restarts. For
  production, consider using auto-unseal with a cloud KMS.
</Callout>
...
<Callout type="info">
  Read replicas are eventually consistent. Use the primary connection for operations that require
...
- [Docker Deployment](/docs/deployment/docker) - Basic Docker deployment guide
