...
impl SqliteTemplateRepo {
    ...
    fn parse_template(row: &super::backend::Row) -> DbResult<Template> {
    ...
    /// Org-scoped WHERE clause for templates reachable within an organization.
...
impl TemplateRepo for SqliteTemplateRepo {
    async fn create(&self, input: CreateTemplate) -> DbResult<Template> {
        ...
        query(
            r#"
            INSERT INTO templates (id, owner_type, owner_id, name, description, content, metadata, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            "#,
        )
        .bind(id.to_string())
    ...
    async fn count_by_owner(
        ...
        let sql = if include_deleted {
            "SELECT COUNT(*) as count FROM templates WHERE owner_type = ? AND owner_id = ?"
        } else {
            "SELECT COUNT(*) as count FROM templates WHERE owner_type = ? AND owner_id = ? AND deleted_at IS NULL"
        };
    ...
    async fn update(&self, id: Uuid, input: UpdateTemplate) -> DbResult<Template> {
        ...
        let sql = format!(
            "UPDATE templates SET {} WHERE id = ? AND deleted_at IS NULL",
            set_clauses.join(", ")
        );
        ...
        let result = query_builder
            ...
            .map_err(map_unique_violation(
                "Template with this name already exists for this owner",
            ))?;
...
mod tests {
    ...
    use super::*;
    use crate::models::TemplateOwner;
    ...
    fn create_template_input(name: &str, content: &str, user_id: Uuid) -> CreateTemplate {
        CreateTemplate {
            owner: TemplateOwner::User { user_id },
            name: name.to_string(),
            description: None,
    ...
    async fn test_create_template() {
        ...
        let input = create_template_input("test-template", "You are a helpful assistant.", user_id);
        let template = repo.create(input).await.expect("Failed to create template");

        assert_eq!(template.name, "test-template");
        assert_eq!(template.content, "You are a helpful assistant.");
        assert_eq!(template.owner_type, TemplateOwnerType::User);
        assert_eq!(template.owner_id, user_id);
    ...
    async fn test_create_template_with_metadata() {
        ...
        assert_eq!(template.description.unwrap(), "A test template");
    }
    ...
    async fn test_create_duplicate_name_fails() {
        ...
        let input = create_template_input("duplicate", "First content", user_id);
        repo.create(input).await.expect("Failed to create template");

        let input2 = create_template_input("duplicate", "Second content", user_id);
    ...
    async fn test_same_name_different_owners_succeeds() {
        ...
        let input1 = create_template_input("same-name", "User 1 content", user1_id);
        let template1 = repo
            .create(input1)
            .await
            .expect("Failed to create template");

        let input2 = create_template_input("same-name", "User 2 content", user2_id);
        let template2 = repo
            .create(input2)
            .await
            .expect("Failed to create template");
        ...
        assert_ne!(template1.owner_id, template2.owner_id);
    }
    ...
    async fn test_get_by_id() {
        ...
        let input = create_template_input("get-test", "Test content", user_id);
        let created = repo.create(input).await.expect("Failed to create template");
    ...
    async fn test_list_by_owner_empty() {
        ...
        let result = repo
            .list_by_owner(TemplateOwnerType::User, user_id, ListParams::default())
            .await
    ...
    #[tokio::test]
    async fn test_list_by_owner_with_templates() {
        let pool = create_test_pool().await;
        let repo = SqliteTemplateRepo::new(pool);
        let user_id = Uuid::new_v4();

        for i in 0..3 {
            let input = create_template_input(&format!("template-{}", i), "Content", user_id);
            repo.create(input).await.expect("Failed to create template");
        }
    ...
    async fn test_list_by_owner_filters_by_owner() {
        ...
        repo.create(create_template_input("user1-template", "Content", user1_id))
            .await
            .expect("Failed to create");
        repo.create(create_template_input("user2-template", "Content", user2_id))
            .await
            .expect("Failed to create");

        let user1_result = repo
            .list_by_owner(TemplateOwnerType::User, user1_id, ListParams::default())
            .await
            .expect("Failed to list");
        let user2_result = repo
            .list_by_owner(TemplateOwnerType::User, user2_id, ListParams::default())
            .await
            .expect("Failed to list");

        assert_eq!(user1_result.items.len(), 1);
        assert_eq!(user1_result.items[0].name, "user1-template");
        assert_eq!(user2_result.items.len(), 1);
        assert_eq!(user2_result.items[0].name, "user2-template");
    }
    ...
    async fn test_count_by_owner() {
        ...
        let count = repo
            .count_by_owner(TemplateOwnerType::User, user_id, false)
            .await
    ...
    async fn test_update_template() {
        ...
        let input = create_template_input("update-test", "Original content", user_id);
    ...
    async fn test_update_no_changes() {
        ...
        let input = create_template_input("no-change", "Original", user_id);
        let created = repo.create(input).await.expect("Failed to create");
    ...
    async fn test_delete_template() {
        ...
        let input = create_template_input("delete-test", "Content", user_id);
        let created = repo.create(input).await.expect("Failed to create");
    ...
    async fn test_count_excludes_deleted() {
        ...
        let template1 = repo
            .create(create_template_input("template-1", "Content", user_id))
            .await
            .expect("Failed to create");
        repo.create(create_template_input("template-2", "Content", user_id))
            .await
            .expect("Failed to create");

        repo.delete(template1.id).await.expect("Failed to delete");
        ...
        let count_all = repo
            .count_by_owner(TemplateOwnerType::User, user_id, true)
            .await
    ...
    async fn test_different_owner_types() {
        ...
        // Create templates for different owner types
        repo.create(CreateTemplate {
            owner: TemplateOwner::Organization {
                organization_id: org_id,
            },
            name: "org-template".to_string(),
            description: None,
            content: "Org content".to_string(),
            metadata: None,
        })
        .await
        .expect("Failed to create org template");

        repo.create(CreateTemplate {
            owner: TemplateOwner::Team { team_id },
            name: "team-template".to_string(),
            description: None,
            content: "Team content".to_string(),
            metadata: None,
        })
        .await
        .expect("Failed to create team template");

        repo.create(CreateTemplate {
            owner: TemplateOwner::Project { project_id },
        ...
        // Verify each owner type only sees their templates
        let org_templates = repo
            .list_by_owner(
            ...
            .await
            .expect("Failed to list");
        assert_eq!(org_templates.items.len(), 1);
        assert_eq!(org_templates.items[0].name, "org-template");

        let team_templates = repo
            .list_by_owner(TemplateOwnerType::Team, team_id, ListParams::default())
            .await
            .expect("Failed to list");
        assert_eq!(team_templates.items.len(), 1);
        assert_eq!(team_templates.items[0].name, "team-template");

        let project_templates = repo
            .list_by_owner(
                TemplateOwnerType::Project,
                project_id,
                ListParams::default(),
            )
            .await
            .expect("Failed to list");
        assert_eq!(project_templates.items.len(), 1);
        assert_eq!(project_templates.items[0].name, "project-template");

        let user_templates = repo
            .list_by_owner(TemplateOwnerType::User, user_id, ListParams::default())
            .await
            .expect("Failed to list");
        assert_eq!(user_templates.items.len(), 1);
        assert_eq!(user_templates.items[0].name, "user-template");
    }
}
