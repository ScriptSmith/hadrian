...
pub struct WasmSqliteDynamicProviderRepo {
    pool: WasmSqlitePool,
}

impl WasmSqliteDynamicProviderRepo {
    ...
    fn parse_provider(row: &WasmRow) -> DbResult<DynamicProvider> {
        ...
        Ok(DynamicProvider {
            ...
            config,
            models,
            is_enabled: row.get::<i32>("is_enabled") != 0,
...
impl DynamicProviderRepo for WasmSqliteDynamicProviderRepo {
    ...
    async fn update(&self, id: Uuid, input: UpdateDynamicProvider) -> DbResult<DynamicProvider> {
        ...
        let mut query = wasm_query(&query_str);
    ...
    async fn delete(&self, id: Uuid) -> DbResult<()> {
        wasm_query("DELETE FROM dynamic_providers WHERE id = ?")
            .bind(id.to_string())
            .execute(&self.pool)
            .await?;
