...
use crate::{
    db::{
        ...
        wasm_sqlite::{WasmParam, WasmSqlitePool, query as wasm_query},
    },
    models::{CreateFile, File, FilePurpose, FileStatus, OBJECT_TYPE_FILE, VectorStoreOwnerType},
};

pub struct WasmSqliteFilesRepo {
    pool: WasmSqlitePool,
}

impl WasmSqliteFilesRepo {
...
impl FilesRepo for WasmSqliteFilesRepo {
    async fn create_file(&self, input: CreateFile) -> DbResult<File> {
        let id = Uuid::new_v4();
        let now = chrono::Utc::now();

        // Note: file_data (blob) is not supported in WASM SQLite; bind as NULL.
        wasm_query(
            r#"
        ...
        .bind(WasmParam::Null) // file_data blob not supported in WASM
    ...
    async fn get_file_data(&self, id: Uuid) -> DbResult<Option<Vec<u8>>> {
        // Blob data is not supported in WASM SQLite. Check if the file exists
        // and return None for the data.
        let result = wasm_query(
            r#"
            SELECT id
            FROM files
