...
pub struct WasmSqliteUsageRepo {
    pool: WasmSqlitePool,
}

impl WasmSqliteUsageRepo {
    pub fn new(pool: WasmSqlitePool) -> Self {
        Self { pool }
    }

    fn media_fields(row: &WasmRow) -> (i64, i64, i64) {
        (
            row.get("image_count"),
...
impl UsageRepo for WasmSqliteUsageRepo {
    ...
    async fn log_batch(&self, entries: Vec<UsageLogEntry>) -> DbResult<usize> {
        ...
        // Use manual BEGIN/COMMIT for transaction (WASM SQLite has no pool.begin())
        wasm_query("BEGIN").execute(&self.pool).await?;
...
fn compute_stats_from_daily_costs(rows: &[WasmRow]) -> DbResult<UsageStats> {
